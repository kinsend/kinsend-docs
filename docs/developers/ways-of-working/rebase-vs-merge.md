...
### Syncing your clones/fork first

You need to keep your clone or fork's `master` (or, respectively, `develop`) branch in sync with the `upstream/master`,
(or respectively, `upstream/develop`). This is also required for rebasing as you will be pointing your `feature` branch
to the `HEAD` of the forked `master` branch. 
...
* Add `upstream` remote which points to `https://github.com/kinsend/kinsend-be`   
  ```linenums="1"
  $ git remote add upstream git@github.com:kinsend/kinsend-be.git
  $ git remote -v
    origin  	git@github.com:your-username/kinsend-be.git (fetch)
    origin  	git@github.com:your-username/kinsend-be.git (push)
    upstream	git@github.com:kinsend/kinsend-be.git (fetch)
    upstream	git@github.com:kinsend/kinsend-be.git (push)
  ```
...
It should now be saying that master is `even with kinsend-be:master`.
...
If you prefer command line, you can execute the commands manually:
